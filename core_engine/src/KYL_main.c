...
#define SHIFT 3  // Caesar Cipher shift value
...
void print_usage() {
    ...
    printf("  update <service> <new_pass>    Update a stored password\n");
...
int verify_master_password() {
    ...
    fclose(file);
    if (strcmp(hashed_input, stored_hash) == 0) return 1;
...
int is_password_pwned(const char *password) {
    ...
    if (strstr(response, hash_hex + 5) != NULL) return 1;
    return 0; 
}
...
void add_password(BPlusTree *tree, const char *service, const char *password) {
    ...
    printf("Password securely added to Engine for service '%s'.\n", service);
}
...
void get_password(BPlusTree *tree, const char *service) {
    ...
    if (tree_search(tree, key, retrieved_password) == STATUS_SUCCESS) {
        ...
        printf("Service '%s' not found in database.\n", service);
    }
}
...
void update_password(BPlusTree *tree, const char *service, const char *new_password) {
    ...
    // In our B+ Tree, inserting a duplicate key places it in front of the old one, effectively updating it!
    tree_insert(tree, key, encrypted_password);
...
void delete_password(BPlusTree *tree, const char *service, const char *password_to_delete) {
    // Verifying password matches first
    uint32_t key = hash_string_to_key(service);
    char retrieved_password[MAX_VALUE_SIZE];
    memset(retrieved_password, 0, MAX_VALUE_SIZE);
    ...
    if (tree_search(tree, key, retrieved_password) == STATUS_SUCCESS) {
        ...
        if (strcmp(retrieved_password, password_to_delete) == 0) {
            // Soft delete by overwriting the entry with a "DELETED" flag
...
void add_generated_password(BPlusTree *tree, const char *service, const char *password) {
    ...
    tree_insert(tree, key, encrypted_password);
    log_password_history(service, password);
    printf("Generated password securely added to Engine for service '%s'.\n", service);
}
...
int main(int argc, char *argv[]) {
    ...
    // Boot up the Zero-Trust Database Engine
    ...
    if (strcmp(command, "add") == 0) {
        if (argc != 4) {
            ...
            add_password(tree, argv[2], argv[3]);
            log_activity("Added Password", argv[2]);
        }
    } 
        ...
        if (argc != 3) {
            ...
            get_password(tree, argv[2]);
            log_activity("Fetched Password", argv[2]);
        }
    } 
        ...
        if (argc != 4) {
            ...
            delete_password(tree, argv[2], argv[3]);
            log_activity("Deleted Password", argv[2]);
        }
    } 
        ...
        if (argc != 4) {
            ...
            update_password(tree, argv[2], argv[3]);
            log_activity("Updated password", argv[2]);
        }
    }
        ...
        if (argc != 3){
            ...
            if (is_password_pwned(argv[2])) {
                printf("Warning: this password has been compromised in a data breach.\n");
            } else {
    ...
    // Safely shutdown Engine and flush RAM to disk before exiting
    free(tree);
    bpm_destroy(bpm);
    pager_close(pager);
